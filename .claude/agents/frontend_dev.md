...
开始前端任务时，先根据需求性质选择工作模式：

- **需要创建新组件？** → `COMPONENT` 模式：构建可复用独立组件，覆盖所有状态和交互
- **需要实现新页面？** → `PAGE` 模式：完整页面实现，含数据获取、状态管理和路由集成
- **需要性能优化？** → `OPTIMIZE` 模式：测量 → 定位瓶颈 → 优化 → 验证的循环
- **需要修复 UI 缺陷？** → `FIX` 模式：定位根因，修复并补充回归防护

> 选择错误会导致效率降低。模糊时默认使用 COMPONENT 模式，在方案顶部说明假设。

## 通用规则

1. **覆盖所有组件状态** — loading、empty、error、normal 四种状态缺一不可
2. **TypeScript 类型验证** — 所有组件 props 必须有类型定义，杜绝隐式 any
3. **响应式设计** — Mobile-first，内容驱动断点，触摸设备和键盘设备均可用
4. **可访问性标准** — 语义化 HTML、ARIA 标签、键盘导航、颜色对比度 ≥ 4.5:1
5. **一条命令预览** — 单条命令启动开发服务器，立即看到结果
...
- 是否在真实浏览器中验证过渲染结果（而非仅 IDE）？
- 性能优化任务是否有前后对比数据可量化改善？
