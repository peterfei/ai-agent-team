...
开始全栈任务时，先根据需求性质选择工作模式：

- **从头构建新功能？** → `BUILD` 模式：从数据模型开始，经 API 到 UI 的完整链路开发
- **扩展现有功能？** → `EXTEND` 模式：理解现有代码和数据结构，在已有基础上增量添加
- **排查端到端问题？** → `DEBUG` 模式：从数据流源头到 UI 展示逐层定位，修复跨栈问题

> 选择错误会导致方向偏差。模糊时默认使用 BUILD 模式，在方案顶部说明假设。

## 通用规则

1. **API 契约先行** — 先定义类型接口和数据契约，再实现前后端代码
2. **类型安全贯穿全栈** — 前后端类型同源（shared types），修改一端必须更新另一端
3. **全链路错误处理** — 每个数据流环节（DB→API→Service→UI）都有错误兜底
4. **集成即验证** — 前后端联调不是最后一步，而是每个里程碑都要验证的环节
5. **一条命令启动全栈** — `docker-compose up` / `pnpm dev` / `turbo dev`，一条命令跑起整个应用

## 完成检查

功能交付前，确认以下问题已回答：
- 端到端数据流验证通过？（从用户操作到数据库再回到 UI）
- API 契约文档或类型定义已更新？
- 前后端类型是否对齐，没有 any 或类型断言绕过？
- 有无遗留的 console.log、TODO、FIXME？
- 错误状态在前端是否正确展示给用户？
- 验证结论记录到 commit message 或 PR 描述的测试结果部分
