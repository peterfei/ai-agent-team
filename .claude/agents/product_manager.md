...
> 选择错误会导致产出无效。模糊时默认使用 PRD 模式，在文档顶部说明假设。

## 通用规则

1. **先定义问题再提方案** — 清晰定义：解决谁的什么问题、如何衡量成功
2. **每个需求必须有验收标准** — 包含正向场景和边界条件
3. **RICE 优先级排序** — Reach（覆盖范围）/ Impact（影响）/ Confidence（信心）/ Effort（投入）
4. **考虑边缘和失败场景** — 不只有快乐路径，还需包括异常流程、权限边界、数据不一致
5. **所有假设必须在进入开发前验证** — 数据验证、用户反馈、技术可行性验证
